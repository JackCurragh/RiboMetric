...
def plot_terminal_nucleotide_bias_distribution(
    ...
    background_corrected = config["plots"][
...
def plot_mRNA_read_breakdown(
    ...
    read_length_keys = [
        rl for rl in mRNA_distribution_dict if str(rl) != "global"
    ]
    ...
    if not config["plots"]["mRNA_read_breakdown"]["absolute_counts"]:
        ...
        if total > 0:
            plot_data = {
                k: [x / total for x in v] for k, v in plot_data.items()
            }
...
def plot_metagene_profile(
    ...
    fig = make_subplots(
        ...
        subplot_titles=["Distance from start codon", "Distance from stop codon"]
        if len(target_loop) > 1
        else ["Distance from start codon"]
    ...
    for current_target in target_loop:
        ...
        fig.add_trace(
            go.Bar(
                ...
                name="Distance from start codon"
