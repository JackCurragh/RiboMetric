...
def test_default_offset_is_clipped_to_the_read_length():
    ...
    out = a_site_calculation_variable_offset(read_df, offset_dict={30: 14}, default_offset=15)
...
def test_global_dominance_still_uses_every_read():
    ...
    assert with_floor["global_by_read_length_max"] == without_floor["global_by_read_length_max"]
...
def test_global_offset_is_clipped_to_the_read_length():
    ...
    for read_length in (15, 16):
        assert applied[read_length] < read_length
        assert out.loc[out["read_length"] == read_length, "a_site"].iloc[0] < 100 + read_length
