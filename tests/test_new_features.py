...
def test_recommend_read_lengths_selects_periodic_abundant_lengths():
    ...
    out = recommend_read_lengths(read_frame, rld, offsets,
                                 min_periodicity=0.5, min_read_proportion=0.05)
...
def test_classify_elongation():
    out = classify_library_type(periodicity=0.8, prop_reads_cds=0.8,
                                start_codon_enrichment_ratio=1.0)
    assert out["label"] == "elongation"


def test_classify_initiation():
    out = classify_library_type(periodicity=0.8, prop_reads_cds=0.8,
                                start_codon_enrichment_ratio=8.0)
    assert out["label"] == "initiation"


def test_classify_low_quality():
    out = classify_library_type(periodicity=0.2, prop_reads_cds=0.2,
                                start_codon_enrichment_ratio=None)
...
def test_gene_body_ramp_empty():
    df = pd.DataFrame(
        columns=["transcript_id", "a_site", "cds_start", "cds_end", "count"]
    )
...
def test_codon_dwell_times_detects_paused_codon():
    ...
    fasta = {"tx1": cds}
    ann = pd.DataFrame({"transcript_id": ["tx1"], "cds_start": [0],
                        "cds_end": [len(cds)]})
    ...
    for i in range(1, 61):
        ...
        rows.append({"transcript_id": "tx1", "reference_name": "tx1",
                     "a_site": a_site, "cds_start": 0, "cds_end": len(cds),
                     "count": weight, "read_name": f"r{i}"})
    df = pd.DataFrame(rows)

    out = compute_codon_dwell_times(df, ann, fasta,
                                    elongation_5_nt=9, elongation_3_nt=9)
...
def test_codon_dwell_times_empty_without_cds_reads():
    from RiboMetric.rust import compute_codon_dwell_times
    df = pd.DataFrame(columns=["transcript_id", "reference_name", "a_site",
                               "cds_start", "cds_end", "count"])
...
def test_floss_flags_aberrant_transcript():
    ...
    out = floss_library_heterogeneity(df, min_reads_per_transcript=20,
                                      floss_cutoff=0.3)
