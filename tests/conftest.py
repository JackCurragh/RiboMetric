...
def sample_annotation_df(test_data_dir):
    ...
    return pd.read_csv(
        ...
        sep="\t",
        dtype={"transcript_id": str, "cds_start": int, "cds_end": int, "transcript_length": int},
    )
...
def sample_metagene_profile():
    ...
    for read_len in [28, 29, 30, 31, 32]:
        profile["start"][read_len] = {i: (100 if i % 3 == 0 else 10) for i in range(-30, 90)}
        profile["stop"][read_len] = {i: (100 if i % 3 == 0 else 10) for i in range(-30, 90)}
    return profile
...
def default_config():
    ...
    return {
        ...
        "plots": {
            "read_frame_distribution": {"upper_limit": 40, "lower_limit": 20},
            "terminal_nucleotide_bias_distribution": {
                "nucleotide_count": 2,
                "background_freq": True,
            },
            "metagene_profile": {"distance_target": "both", "distance_range": [-50, 50]},
            "mRNA_distribution": {"absolute_counts": False},
        },
        "qc": {
            "use_cds_subset": {"read_frame_distribution": True},
            "read_frame_distribution": {"3nt_count_cutoff": 0.05},
            "cds_coverage": {"in_frame_coverage": True},
        },
        "metrics": {
            "default": [
