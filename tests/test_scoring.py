...
def test_config_override_changes_gate_and_threshold():
    config = {
        "scoring": {
            "fragment_uniqueness_score": {"gate": True, "status": {"pass": 0.95, "warn": 0.9}}
        }
    }
...
@pytest.mark.parametrize("metric_key,raw_good,raw_bad", DIRECTION_CASES)
def test_registry_direction_matches_the_scoring_behaviour(metric_key, raw_good, raw_bad):
...
def test_registry_and_scoring_spec_agree():
    ...
    from_spec = {score_key: entry["metric"] for score_key, entry in DEFAULT_SCORING.items()}
...
def test_context_metrics_are_never_scored():
    ...
    for key, spec in METRIC_REGISTRY.items():
        if spec.direction == CONTEXT:
            assert spec.scored_as is None, f"{key} is context-dependent and must not be scored"
