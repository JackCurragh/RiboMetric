...
def test_plots():
    ...
    if not is_plotly_html(
        plot_read_length_distribution(read_length_distribution(read_df), config)["fig_html"]
    ):
        errors.append("Read length distribution plot html output error")
    if not is_plotly_html(
        plot_terminal_nucleotide_bias_distribution(
            terminal_nucleotide_bias_distribution(read_df), config
        )["fig_html"]
    ):
        errors.append("Ligation bias distribution plot html output error")
    if not is_plotly_html(
        plot_nucleotide_composition(nucleotide_composition(sequence_data_single), config)[
            "fig_html"
        ]
    ):
        errors.append("Nucleotide composition plot html output error")
    if not is_plotly_html(
        plot_read_frame_distribution(read_frame_distribution(a_site_calculation(read_df)), config)[
            "fig_html"
        ]
    ):
