.PHONY: help clean clean-build clean-pyc clean-test lint format typecheck test test-cov \
        coverage build dist preflight docs docs-strict servedocs install \
...
help:
	...
	@echo "docs-strict  build the docs with warnings fatal"
...
# Invokes sphinx directly rather than delegating to docs/Makefile, which uses
# a bare `python`/`sphinx-build` and dies with "command not found" whenever
# those are not on PATH -- the same failure this file's $(PYTHON) rule exists
# to avoid. The API reference is generated by a builder-inited hook in
# docs/conf.py, so it comes out identically here and on Read the Docs.
docs:
