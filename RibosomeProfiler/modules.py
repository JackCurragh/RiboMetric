...
def ligation_bias_distribution(read_df: pd.DataFrame, num_bases: int = 2, five_prime: bool = True) -> dict:
    '''
    ...
    '''
    if five_prime:
        two_sequence_dict = dict(read_df['sequence'].str.slice(stop=num_bases).value_counts(normalize=True).sort_index())
    else:
        two_sequence_dict = dict(read_df['sequence'].str.slice(start=-num_bases).value_counts(normalize=True).sort_index())
